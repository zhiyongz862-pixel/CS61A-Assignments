...
一些小tips
+ lab02当中了解到python当中的and是一个抓取装置


# CS61A 笔记：自递归函数 (Recursive Functions)

### 1. 核心定义
**递归 (Recursion)**：一个函数在内部调用了它自己。
> **你的理解：** “想让功能一直持续下去”。
> **学术修正：** “将大问题分解为与原问题相似的小问题，直到触达边界”。

### 2. 递归的三大组成部分 (必背)
想要递归不崩溃，必须具备以下三要素：

1.  **Base Case (基准情况/刹车)**：
    - 最简单、不需要递归就能直接得出答案的情况。
    - *没有它，你的“无限持续”就会变成“程序崩溃”。*
2.  **Recursive Call (递归调用)**：
    - 函数调用自身。
3.  **Making Progress (向目标迈进)**：
    - 每次调用时，传入的参数必须比上次更“接近”基准情况。
...
def repeat_print():
    print("功能持续中...")
    repeat_print()  # 没有任何条件阻止它，会一直跑直到报错