def shuffle(s):
    ...
    s = list(s)
    ans = []
    first,second = s[0:len(s)//2],s[len(s)//2:len(s)]
    f_i,s_i = iter(first),iter(second)
...
def balanced(m):
    ...
    "*** YOUR CODE HERE ***"
    if is_mobile(m):
        left_total,right_total= total_mass(end(left(m))) * length(left(m)),total_mass(end(right(m)) )* length(right(m))
